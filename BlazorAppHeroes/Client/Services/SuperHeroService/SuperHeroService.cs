...
namespace BlazorAppHeroes.Client.Services.SuperHeroService
{
    ...
    
    {
        ...
        {
            var result = await _http.GetFromJsonAsync<List<Comic>>("api/superhero/comics");//nombre del controlador
            if (result != null)
                Comics = result;
        }
        ...
        {
            var result = await _http.GetFromJsonAsync<SuperHero>($"api/superhero/{id}");//nombre del controlador
            if (result != null)
                return result;
        ...
        {
            var result = await _http.GetFromJsonAsync<List<SuperHero>>("api/superhero");//nombre del controlador
            if (result != null)
                Heroes = result;
        }
